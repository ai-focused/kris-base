...
def wp_join():
    ...
    result = r.json()
    _append_circuit(circuit_name or f"Circuit {result['circuit_id'][-6:]}", server, result["circuit_id"], name, result["token"])
...
def wp_create_circuit():
    ...
    _append_circuit(circuit_name, server, circuit["circuit_id"], your_name, particle["token"])

    return jsonify({"status": "created", "circuit_id": circuit["circuit_id"], "circuit_name": circuit_name})


def _append_circuit(name, server, circuit_id, my_name, token):
    global _active_idx
    existing = {"circuits": []}
    if _sync_json_path and _sync_json_path.exists():
        try:
