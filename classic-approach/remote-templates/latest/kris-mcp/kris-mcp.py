...
Entry point (v3.7+):
    .kris/kris-mcp/.venv/bin/python3 .kris/kris-mcp/kris-mcp.py

Runs from the project root (where memory-bank/ and .kris/ live as siblings).
"""
...
# v3.7: kris-ui and kris-mcp live in .kris/ (not under memory-bank/) so no
# exclusion needed — memory-bank/ is exclusively ring documentation.
...
if _wp:
    ...
    def wp_status() -> list[dict]:
        ...
        v3.7: each particle object gains a `self: true` field on the entry
        matching the caller's own particle_id (read from .kris/sync.json
        me.id). If me.id is missing (legacy sync.json), no flag is added.
        """
        r = _client.get("/particles/presence")
        r.raise_for_status()
        particles = r.json()
        my_id = _wp.get("particle_id", "")
        if my_id and isinstance(particles, list):
            for p in particles:
                if isinstance(p, dict) and p.get("id") == my_id:
                    p["self"] = True
        return particles
