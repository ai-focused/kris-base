...
def _discover_memory_bank() -> Path:
    """Find memory-bank/ relative to this script.

    v3.7: kris-ui.py lives at .kris/kris-ui/kris-ui.py — memory-bank/ is a sibling of .kris/
    v3.6 legacy: kris-ui.py lives at memory-bank/kris-ui/kris-ui.py — memory-bank/ is the parent

    Walk up from script_dir looking for a directory that contains memory-bank/ as a child.
    """
    env = os.environ.get("KRIS_MEMORY_BANK")
    if env:
        return Path(env).resolve()
    script_dir = Path(__file__).resolve().parent
    for parent in script_dir.parents:
        candidate = parent / "memory-bank"
        if candidate.is_dir():
            return candidate.resolve()
    # Last-resort fallback: v3.6 layout
    return script_dir.parent.resolve()


MEMORY_BANK_ROOT = _discover_memory_bank()
    ...
    try:
        ...
        if since_date:
            ...
            if git_diff.returncode == 0:
                ...
                # v3.7: kris-ui lives in .kris/kris-ui, not under memory-bank/,
                # but it's gitignored so git log won't report its files anyway.
                # Keep this filter defensively for v3.6 projects that may still
                # have memory-bank/kris-ui/ around during migration.
                changed_files = sorted(
                    f for f in all_files
                    if not f.startswith("memory-bank/kris-ui")
                    and not f.startswith(".kris/")
                )
